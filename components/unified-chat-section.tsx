...
export function UnifiedChatSection({ user }: UnifiedChatSectionProps) {
  const [messages, setMessages] = useState<ChatMessage[]>([
    ...
    },
    {
      id: "5",
      user: "Model Sarah",
      message: "Thanks Alice! Hope you're enjoying the show! ✨",
      ...
      <div className="p-4 border-b border-gray-200 bg-white/80 backdrop-blur-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-gray-900 tracking-tight">Live Chat</h3>
          ...
          {messages.map((message) => (
            <div key={message.id} className="space-y-2">
              <div className="flex items-center space-x-2 text-xs">
                <span className={`font-bold ${message.isModel ? "text-purple-600" : "text-gray-700"}`}>
                  {message.user}
                  {message.isModel && (
